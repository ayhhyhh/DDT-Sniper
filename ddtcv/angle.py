...
_default_onnx_model_fp = os.path.join(
    _static_dir, "model/angle_1_rec_en_number_lite/angle_1_rec_en_number_lite.onnx"
)
_default_character_dict_fp = os.path.join(
    _static_dir, "model/angle_1_rec_en_number_lite/angle_dict.txt"
)
...
def rec_angle(
    angle_image: np.ndarray, rec_model: ONNXModel = _default_wind_rec_model
) -> int:
    return int(rec_model.predict(angle_image))


def save_angle_image(
    filename: str,
    image: np.ndarray,
    x_slice: slice = default_x_slice,
    y_slice: slice = default_y_slice,
